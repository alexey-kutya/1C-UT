...
//	MOD Нач Кутья А.А.			
			Если Роли <> Неопределено Тогда
				ЕстьРольПолныеПрава = НЕ Роли.Найти("ПолныеПрава") = Неопределено;
				ЕстьРольАдминистраторСистемы = НЕ Роли.Найти(Пользователи.РольАдминистратораСистемы().Имя) = Неопределено;
			Иначе
				ЕстьРольПолныеПрава = Ложь;
				ЕстьРольАдминистраторСистемы = Ложь;
			КонецЕсли;
//	MOD Кон Кутья А.А.			
...
//	MOD Нач Кутья А.А.			
				ИЛИ НЕ ЕстьРольПолныеПрава
				ИЛИ НЕ ЕстьРольАдминистраторСистемы Тогда
//	MOD Кон Кутья А.А.			
...
Процедура ЗапомнитьСвойстваПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	//КутьяАА для программной записи
	ЗначениеСвойства = Неопределено;
	ПользовательОбъект.ДополнительныеСвойства.Свойство("ИдентификаторПользователяИБ",ЗначениеСвойства);
	Если НЕ ЗначениеСвойства = Неопределено Тогда
		СтарыйПользователь.ИдентификаторПользователяИБ = ЗначениеСвойства;
	КонецЕсли; 
	//КутьяАА
