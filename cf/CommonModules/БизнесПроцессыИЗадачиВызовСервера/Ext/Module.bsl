	...
	Для Каждого Задача Из СведенияОЗадачах Цикл
		...
		Если Задача.Значение.Выполнена Тогда
			...
			Если ТолькоПроверка Тогда
				Прервать;
			КонецЕсли;
//	КутьяАА	если задача выполнена, ничего не надо делать		
			Продолжить;
//	КутьяАА			
		КонецЕсли;	
		...
		Попытка
			...
			Если Не Задача.Значение.БизнесПроцесс.Пустая() Тогда
...
//	КутьяАА
			МассивАдресовКопии = Новый  Массив;
			АдресИсполнителя = ЗадачаОбъект.БизнесПроцесс.Исполнитель.ФизЛицо.АдресЭлектроннойПочты;
			МассивАдресовКопии.Добавить(АдресИсполнителя);
			
			Если ЗадачаОбъект.БизнесПроцесс.НаПроверке Тогда
				АдресПроверяющего = ЗадачаОбъект.БизнесПроцесс.Проверяющий.ФизЛицо.АдресЭлектроннойПочты;
				МассивАдресовКопии.Добавить(АдресПроверяющего);
			КонецЕсли; 
			
			АдресАвтора = ЗадачаОбъект.Автор.ФизЛицо.АдресЭлектроннойПочты;
			
			Если АдресАвтора <> АдресИсполнителя И АдресАвтора <> АдресПроверяющего Тогда
				МассивАдресовКопии.Добавить(АдресАвтора);
			КонецЕсли; 
			
			ТИ.СформироватьИОтправитьУведомление(НоваяЗадача.Ссылка, Справочники.ШаблоныСообщений.ПеренаправлениеЗадачи, ИнфоОПеренаправлении.Исполнитель.ФизЛицо.АдресЭлектроннойПочты, ИнфоОПеренаправлении.Комментарий, Задача.Ключ.Исполнитель.ФизЛицо,,,,,МассивАдресовКопии);
//	КутьяАА			
...
// Устарела. Следует использовать ФормаВыполненияЗадачи.
//
// Получить структуру с описанием формы выполнения задачи.
//
// Параметры
...
Функция ПолучитьФормуВыполненияЗадачи(Знач ЗадачаСсылка) Экспорт
	...
	Возврат ФормаВыполненияЗадачи(ЗадачаСсылка);
