...
КонецФункции // ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита)()

&НаСервере
Функция НайтиПользователяПоРеквизиту(ИмяРеквизита, ЗначениеРеквизита) Экспорт 

	Возврат Справочники.Пользователи.НайтиПоРеквизиту(ИмяРеквизита, ЗначениеРеквизита);

КонецФункции // НайтиПользователяПоРеквизиту(ФизЛицо, "ФизЛицо")()
		...
		Если ВыборкаВидОшибкиОбмена.КоличествоПопытокОбмена > КоличествоПопытокОбменаКонстанта Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если ВыборкаДетальныеЗаписи.КоличествоПопытокОбмена > КоличествоПопытокОбменаКонстанта Тогда
					...
					ТекстОбращения = ТекстОбращения + "<P>" + Объект+" ("+Объект.BaseID+")"+"</P>";
...
Процедура ПередатьНаСервере(СтрокаXML, ЧислоВыгруженныхОбъектов, Узел, ПараметрыWS)
	...
	ТекстОшибки = "";
	Попытка
		ЗагруженоОбъектов = Прокси.ПринятьДанные(Данные, ТекстОшибки, Истина);
