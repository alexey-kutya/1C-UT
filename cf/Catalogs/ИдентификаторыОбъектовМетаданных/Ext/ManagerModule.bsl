...
// Только для внутреннего использования.
Процедура ПроверитьОбъектПередЗаписью(Объект, АвтоматическоеОбновление = Ложь) Экспорт
	...
	Если НЕ АвтоматическоеОбновление Тогда
		...
		ИначеЕсли ПолноеИмяИспользуется(Объект.ПолноеИмя, Объект.Ссылка) Тогда
	...
	Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если Объект.ЭтоНовый()
		   И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового()) Тогда
		...
		Если Не Объект.ПометкаУдаления
		   И Не ЭтоКоллекция(Объект.Ссылка) Тогда
...
// Только для внутреннего использования.
Процедура ВызватьИсключениеПоОшибке(ТекстОшибки) Экспорт
...
Функция ПолноеИмяИспользуется(ПолноеИмя, КромеИдентификатора = Неопределено)
