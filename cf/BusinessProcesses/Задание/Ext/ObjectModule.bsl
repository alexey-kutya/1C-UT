...
//	MOD Кутья АА
		Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") Тогда
			...
			Если НомерИтерации > 1 И НЕ Выполнено И НЕ Подтверждено Тогда //Возврат
				
				СообщениеСтруктура = ТИ.СформироватьЗаголовокИТекстСообщения(Задача, Справочники.ШаблоныСообщений.ДоработкаВнешнейОбработкиТекстЗадачи, КомментарийПоЗадаче);
				Задача.Наименование = СообщениеСтруктура.ЗаголовокПисьма;
				Задача.Описание = СообщениеСтруктура.ТекстПисьма;
				ТИ.СформироватьИОтправитьУведомление(Задача, Справочники.ШаблоныСообщений.ДоработкаВнешнейОбработки, 
					Исполнитель.ФизЛицо.АдресЭлектроннойПочты, КомментарийПоЗадаче);
				
			ИначеЕсли НомерИтерации = 1 И НЕ Выполнено И НЕ Подтверждено Тогда
				
				СообщениеСтруктура = ТИ.СформироватьЗаголовокИТекстСообщения(Задача, Справочники.ШаблоныСообщений.ДобавлениеВнешнейОбработкиТекстЗадачи, КомментарийПоЗадаче,,Исполнитель, Проверяющий);
				Задача.Наименование = СообщениеСтруктура.ЗаголовокПисьма;
				Задача.Описание = СообщениеСтруктура.ТекстПисьма;
				ТИ.СформироватьИОтправитьУведомление(Задача, Справочники.ШаблоныСообщений.ДобавлениеВнешнейОбработки, Исполнитель.ФизЛицо.АдресЭлектроннойПочты);
				
			КонецЕсли;
		ИначеЕсли ТипЗнч(Предмет) = Тип("ДокументСсылка.ИзменениеКонфигурации") Тогда
			Если Релиз.Владелец = Справочники.Конфигурации.ЗарплатаИУправлениеПерсоналом Тогда
			...
			СообщениеСтруктура = ТИ.СформироватьЗаголовокИТекстСообщения(Задача, Шаблон, КомментарийПоЗадаче,,Автор, Исполнитель);
...
//		Задача.СрокИсполнения = СрокИсполненияЗадачиДляПроверки();
		Задача.СрокИсполнения = ТекущаяДата()+ 86400*7;
		Задача.Важность = Важность;
		Задача.Предмет = Предмет;
		
//	MOD Кутья АА
		Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Обращение") Тогда
			СообщениеСтруктура = ТИ.СформироватьЗаголовокИТекстСообщения(Задача, Справочники.ШаблоныСообщений.ТестированиеВнешнейОбработкиТекстЗадачи, КомментарийПоЗадаче);
			Задача.Наименование = СообщениеСтруктура.ЗаголовокПисьма;
			Задача.Описание = СообщениеСтруктура.ТекстПисьма;
			Задача.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоТестированию.ПередачаНаТестирование");
		КонецЕсли;
//	MOD Кутья АА
