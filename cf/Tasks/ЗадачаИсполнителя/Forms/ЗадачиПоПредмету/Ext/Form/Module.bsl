...
&НаКлиенте
Процедура ВыгрузитьПовторно(Команда)
	
	Задача = Элементы.Список.ТекущиеДанные.Ссылка;
	Действие = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоТестированию.ПередачаНаТестирование");
	Если ТИ.ПолучитьРеквизитПоСсылке(Задача, "Операция") = Действие Тогда
		ТИ.ЗарегистрироватьИзменение(Задача, Действие);
	Иначе
		Сообщить("Для данной задачи выполнить нельзя.");
	КонецЕсли;
